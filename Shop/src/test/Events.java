...
public class Events extends HttpServlet {
	...
	private void generatePage(HttpServletResponse response,HttpServletRequest request) throws IOException {
		...
		out.println("<html><head><link rel='Stylesheet' type='text/css' href='style.css' /><title>NADCHODZACE ZAWODY</title></head><body class = 'back'><h2>NADCHODZACE ZAWODY</h2>");
		try {
			generateTable(out);
	...
	private void generateProduct(PrintWriter out,String title,String place,String day,int length,int count,String login){
		...
		out.println("<input type='hidden' name = 'action' value='Take'>");
		out.println("<input class = 'button' type='submit' value='Take Part!'>");
		out.println("</form></td>");
	}
